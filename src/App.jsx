...
function applySeo(location) {
  ...
  const imageUrl = post?.image ? new URL(post.image, window.location.origin).toString() : new URL("/assets/categories/technical-practice.png", window.location.origin).toString();
...
function SiteHeader({ onNavigate, animate = false }) {
  ...
  return <header className={animate ? "site-header header-enter" : "site-header"}>
    <a className="brand" href="/" onClick={(event) => { event.preventDefault(); setMenuOpen(false); onNavigate("/"); }}>咕咕嘎嘎的个人博客</a>
...
function HomePage({ onNavigate }) {
  ...
  return <main>
    ...
    <div className="page-shell">
      ...
      <section className="about-card" id="about" data-reveal><img src="/assets/backgrounds/about-cs.png" alt="深夜学习与机器人原型的桌面" /><div className="about-content"><p className="eyebrow dark-label">ABOUT THIS BLOG</p><h2>关于这个博客</h2><p>这里主要分享计算机学习、技术文章和个人项目，也记录那些值得复盘的实现过程。</p><ArrowLink onClick={() => onNavigate("/articles")}>浏览文章索引</ArrowLink></div></section>
